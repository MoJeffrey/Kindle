...
public class KindleGenOPF {
	private File TemplateForOPF=new File("metadata.opf");
	...
	private void CreateFileAndWriteForReWriteXHTMLString() {
		byte[] ReWriteOPFByte = ReWriteOPFString.getBytes();  
		...
		for(int x=0;x<ComicPictureFileList.length;x++){
			if(PageInt==0) {
				...
				SBInsertContent.insert(ContentIndex,
						"<item id=\""+"id"+(id++)+"\" href=\""+ComicPictureFileList[x]+"\" media-type=\"image/jpeg\" properties=\"cover-image\"/>"+"\r\n");
				
				ContentIndex=SBInsertContent.lastIndexOf(OPFForManifest);
				SBInsertContent.insert(ContentIndex,
						"<item id=\""+"id"+(id++)+"\" href=\""+ProfilesForXHTMLFileList.get(x)+"\" media-type=\"application/xhtml+xml\"/>"+"\r\n");
				
				PageInt++;
			}else{
				int ContentIndex=SBInsertContent.lastIndexOf(OPFForManifest);
				SBInsertContent.insert(ContentIndex,
						"<item id=\""+"id"+(id++)+"\" href=\""+ComicPictureFileList[x]+"\" media-type=\"image/jpeg\"/>"+"\r\n");
	...
	private String ReadTemplateForXHTML() {
        ...
        try {
            ...
            reader = new InputStreamReader(new FileInputStream(TemplateForOPF));
            int tempchar;
